...
import com.milunas.scaladdd.library.commons.events.JustForwardPublisher
...
class MainModule {
  ...
  def program()(implicit concurrentEffect: ConcurrentEffect[IO], timer: Timer[IO]): IO[Unit] = {
    for {
      ...
      patronModule <- PatronModule.createSync(bookModule.bookRepository, new JustForwardPublisher())
